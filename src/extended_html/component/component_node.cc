...
// TODO: free mem
static gfx::Font* component_node_default_font = nullptr;
static gfx::PlatformFont* component_node_customPlatformFont = nullptr;

/// \todo use fontsRegistry
// FontList for the texts except for the header.
static gfx::FontList component_node_GetTextFontList() {
  DCHECK(component_node_default_font);
  DCHECK(component_node_default_font->platform_font());
  DCHECK(component_node_default_font->platform_font()->GetFontSize());
  const int size_delta = 0; // size in pixels to add
  gfx::Font font = component_node_default_font->Derive(size_delta, gfx::Font::NORMAL,
...
static void component_node_prepareUIFonts() {
  if(!sktpForUI) {
  ...
  DCHECK(sktpForUI);
  // TODO: free mem
  if(!component_node_customPlatformFont) {
    printf("creating component_node_customPlatformFont\n");
    ...
    component_node_customPlatformFont = new gfx::PlatformFontSkia(
  ...
  if(!component_node_default_font) {
    printf("creating component_node_default_font\n");
    // TODO: free mem
    DCHECK(component_node_customPlatformFont);
    component_node_default_font = new gfx::Font(component_node_customPlatformFont);
  }
...
void ComponentNode::RenderTreeNodeVisit(const NodeVisitor *render_target) {
  ...
  component_node_prepareUIFonts(); // see DCHECK(sktpForUI);
  DCHECK(component_node_default_font);

  const gfx::FontList& font_list = component_node_GetTextFontList();
  DCHECK(component_node_default_font);
  ...
  if(!ui::ResourceBundle::HasSharedInstance()) {
    printf("component_node: creating ui::ResourceBundle\n");
  ...
  // TODO
  //std::unique_ptr<cobalt::render_tree::component_node_ContainerView>
  //  component_node_container_;
  //auto& component_node_container_
  //  = custom_generating_node_->component_node_container_;
  auto& component_node_container_
    = custom_generating_node_->component_node_container_;

  if(!component_node_container_) {
    printf("component_node: creating container\n");
    component_node_container_ = std::make_unique<component_node_ContainerView>(this);
    /*component_node_container_->SetBounds(
      sk_rect_transformed.x(), sk_rect_transformed.y(),
      sk_rect_transformed.width(), sk_rect_transformed.height());*/
    component_node_container_->SetSize(
      gfx::Size(sk_rect.width(), sk_rect.height()));
    component_node_container_->SetPreferredSize(
      gfx::Size(sk_rect.width(), sk_rect.height()));

    component_node_container_->SetLayoutManager(std::make_unique<views::FillLayout>());

    /// if(!component_node_container_->has_children) {
    ///   component_node_container_->addChildren();
    ///   component_node_container_->has_children = true;
    /// }

    // see https://github.com/blockspacer/skia-opengl-emscripten/blob/master/src/chromium/ui/views/examples/textfield_example.cc
    //auto layout_manager = std::make_unique<views::GridLayout>(component_node_container_);
    ...
    //component_node_container_->addChildren(layout_manager.get());
    //component_node_container_->SetLayoutManager(std::move(layout_manager));
  ...
  auto& component_node_widget_
    = custom_generating_node_->component_node_widget_;

  if(!component_node_widget_) {
    printf("component_node: creating widget\n");
    component_node_widget_ = std::make_unique<views::Widget>();
    ...
    //params.delegate = component_node_container_;
    ///params.delegate =
    ///    new ExamplesWindowContents(std::move(on_close), std::move(examples));
    ///params.context = window_context;
    component_node_widget_->Init(params);
    //component_node_widget_->Show();

    component_node_widget_->SetFullscreen(/*true*/false);
    component_node_widget_->SetVisibleOnAllWorkspaces(true);

    DCHECK(component_node_container_);

    /// \TODO handle memory properly!
    component_node_widget_->SetContentsView(component_node_container_.get());
  }

  DCHECK(component_node_widget_);

  component_node_widget_->SetSize(
    gfx::Size(transformed_rect.width(), transformed_rect.height()));
  component_node_widget_->SetBounds(
    gfx::Rect(
     sk_rect_transformed.x(), sk_rect_transformed.y(),
     transformed_rect.width(), transformed_rect.height()));

  views::View* root_view = component_node_widget_->GetRootView();
  DCHECK(root_view);
  if(!root_view->Contains(component_node_container_.get())) {

    /// \TODO handle memory properly!
    root_view->AddChildView(component_node_container_.get());

    printf("added component_node_container_ to root_view\n");
  }
  DCHECK(root_view->GetEffectiveViewTargeter());

  //component_node_container_->SetVisible(true);
  component_node_container_->SetEnabled(true);
  ...
  //component_node_container_->SetNativeTheme(nativeThemeAura);
  component_node_container_->SetBackground(views::CreateSolidBackground(
        blink::Color(0.9f, 1.0f, 1.0f, 0.9f).Rgb()));

  //component_node_container_->SetBorder(views::CreateSolidBorder(2, SK_ColorBLUE));
  const gfx::Insets child_margins(1, 1);
  // TODO: free mem?
  //component_node_container_->SetProperty(views::kMarginsKey,
  //  new gfx::Insets(child_margins));

  //cc::layer_tree_host()

  //component_node_container_->SetPaintToLayer(ui::LAYER_TEXTURED);

  //blink::Path path;
  //path.AddRect({0, 0, 800, 800});
  //component_node_container_->set_clip_path(path.GetSkPath());

  component_node_container_->set_owned_by_client(); // prevents view_to_be_deleted by parent

  //component_node_container_->InvalidateLayout();
  //component_node_container_->SizeToPreferredSize();
  //component_node_container_->Layout();
  //DCHECK(component_node_container_->GetLocalBounds() == gfx::Rect(0, 0, 800, 800));
  component_node_container_->SchedulePaint();
  component_node_container_->
    SetFocusBehavior(views::View::FocusBehavior::ALWAYS);
  DCHECK(component_node_container_->IsFocusable());

  //root_view->SetBoundsRect(component_node_container_->GetLocalBounds());
  ...
  component_node_widget_->SetFullscreen(/*true*/false);
  component_node_widget_->Maximize();
  component_node_widget_->Show(); // TODO: component_node_widget_.Close();
  component_node_widget_->Activate(); // TODO: wait?
  ...
  DCHECK(component_node_container_);
  DCHECK(component_node_container_->m_ComponentNode);
  DCHECK(component_node_container_->textfield_);
  component_node_container_->textfield_->
    SetFontList(font_list.Derive(
      -size_delta, gfx::Font::UNDERLINE, gfx::Font::Weight::BOLD));

  component_node_container_->textfield_->SetColor(
    blink::Color(0.0f, 1.0f, 0.5f, 0.5f).Rgb());
  /*component_node_container_->textfield_->GetRenderTextForSelectionController()->
  ...
  //component_node_container_->textfield_->GetRenderTextForSelectionController()->
  //  SetSelectable(true);
  component_node_container_->textfield_->SetSelectionBackgroundColor(
    blink::Color(0.1f, 0.2f, 0.0f, 0.5f).Rgb());
  component_node_container_->textfield_->SetSelectionTextColor(
    blink::Color(0.4f, 0.4f, 0.9f, 0.5f).Rgb());

  component_node_container_->textfield_->ChangeTextDirectionAndLayoutAlignment(
    base::i18n::LEFT_TO_RIGHT);

  /*gfx::Range text_range, selection_range;
  base::string16 text;
  if (component_node_container_->textfield_->
        ...
        GetEditableSelectionRange(&selection_range))
  {
    ...
    component_node_container_->textfield_->SelectRange(text_range);
  }*/
  ...
  DCHECK(component_node_widget_->GetContentsView());
  ...
  DCHECK(custom_generating_node_);
  {
    ...
    {
      //printf("HTMLComponentElementID_ = %i\n",
      //    custom_generating_node_->
      //    HTMLComponentElementID_);
...

  {
    ...
    for(HTMLComponentElement::ScheduledMouseEvent& mEv :
          custom_generating_node_->
          scheduledEvents_
          .scheduledMouseEvents_)
    {
      ...
      component_node_widget_->OnMouseEvent(&mEvTransformed);
    }
    custom_generating_node_->
  ...
  component_node_widget_->GetRootView()->SchedulePaint();
  component_node_container_->SchedulePaint();
  component_node_widget_->GetRootView()->SchedulePaint();

  /*views::View* root_view = component_node_widget_->GetRootView();
  DCHECK(root_view);
  if(!root_view->Contains(component_node_container_)) {
    root_view->AddChildView(component_node_container_);
    printf("added component_node_container_ to root_view\n");
  }
  DCHECK(root_view->GetEffectiveViewTargeter());*/
  ...
  //gfx::Size component_node_container_size(800, 800/*component_node_container_->GetPreferredSize()*/);
  gfx::Size component_node_container_size =
    root_view->GetMirroredBounds().size();

    //component_node_container_->GetMirroredBounds().size();
  DCHECK(!component_node_container_size.IsEmpty());

  ui::CanvasPainter canvasPainter =
    ui::CanvasPainter(&bitmap, component_node_container_size,
  ...
  views::PaintInfo component_node_container_paint_info = views::PaintInfo::CreateRootPaintInfo(
      ui::PaintContext(list.get(), 1.f, /* invalidation */ first_paint, /*true*/ false),
      //canvasPainter.context(),
      component_node_container_size);
  DCHECK(!component_node_container_paint_info.paint_recording_bounds().IsEmpty());
  DCHECK(component_node_container_);

  //component_node_container_->Paint(component_node_container_paint_info);
  //ui::Compositor* compositor = component_node_widget_->GetCompositor();
  //DCHECK(compositor);
  ui::PaintContext component_node_container_paint_context(
                             list.get(),
                             1.f,//compositor->device_scale_factor(),
                             //gfx::Rect(0, 0, 800, 800),//
                             gfx::Rect(component_node_container_size),
                             false /*true*/);

  //component_node_widget_->GetRootView()->OnPaintLayer(component_node_container_paint_context);
  //component_node_widget_->GetRootView()->OnPaint(&gfx_canvas);
  //component_node_widget_->GetRootView()->PaintDebugRects(component_node_container_paint_info);

  // TODO
  // DCHECK(component_node_widget_->GetCompositor());

  component_node_widget_->GetRootView()->
    PaintFromPaintRoot(component_node_container_paint_context);
  ...
  gfx::ImageSkia tmp_image(gfx::ImageSkiaRep(paintRecord,
    gfx::ScaleToCeiledSize(component_node_container_size, 1.0), 1.0));
...
void ComponentNode::Accept(NodeVisitor* visitor) {
  visitor->Visit(this);
}

math::RectF ComponentNode::GetBounds() const { return data_.rect; }

void cobalt::render_tree::ComponentNode::SetCustomGeneratingNode(
  HTMLComponentElement* custom_generating_node)
{
