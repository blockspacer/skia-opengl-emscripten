...
using render_tree::ComponentNode;
...
Component::Component(
      ...
      content_size_(content_size)
  {
    ...
    printf("created new Component\n");
  }

WrapResult Component::TryWrapAt(
...
void Component::SplitBidiLevelRuns() {}

bool Component::TrySplitAtSecondBidiLevelRun() { return false; }

base::Optional<int> Component::GetBidiLevel() const {
  return paragraph_->GetBidiLevel(text_position_);
}

void Component::SetShouldCollapseLeadingWhiteSpace(
    bool /*should_collapse_leading_white_space*/) {
  // Do nothing.
}

void Component::SetShouldCollapseTrailingWhiteSpace(
    bool /*should_collapse_trailing_white_space*/) {
  // Do nothing.
}

bool Component::HasLeadingWhiteSpace() const { return false; }

bool Component::HasTrailingWhiteSpace() const { return false; }

bool Component::IsCollapsed() const { return false; }

bool Component::JustifiesLineExistence() const { return true; }

bool Component::AffectsBaselineInBlockFormattingContext() const {
  return false;
}

LayoutUnit Component::GetBaselineOffsetFromTopMarginEdge() const {
...
void AddLetterboxedInputToRenderTree(
    ...
    //const Component::SetBoundsCB& set_bounds_cb,
    //const Component::GetInputAnimCB& replace_input_animation_cb,
    CompositionNode::Builder* border_node_builder) {
  DCHECK(component);

  if (dimensions.image_rect) {
    ComponentNode::Builder builder(*(dimensions.image_rect)
                                           //, set_bounds_cb
                                           );
   ...
   /*if (builder.animation && builder.animation->duration()) {
    ...
    ComponentNode* newNode = new ComponentNode(builder);
    DCHECK(component);
    DCHECK(component->custom_generating_node_);
    newNode->SetCustomGeneratingNode(component->custom_generating_node_);
...
void Component::RenderAndAnimateContent(
    CompositionNode::Builder* border_node_builder,
...
void Component::UpdateContentSizeAndMargins(
  ...
  if (!maybe_height) {
    LOG(ERROR) << "Component element has computed height \"auto\"!";
  }
  if (!maybe_width) {
    LOG(ERROR) << "Component element has computed width \"auto\"!";
  }
  ...
  if (!maybe_height || !maybe_width) {
    LOG(ERROR)
        << "Cobalt Component does not handle \"auto\" dimensions correctly! "
...
void Component::DumpProperties(std::ostream* stream) const {
...
void Component::RenderAndAnimateContentWithLetterboxing(
...
void cobalt::layout::Component::SetCustomGeneratingNode(HTMLComponentElement *custom_generating_node)
{
  DCHECK(custom_generating_node);
  custom_generating_node_ = custom_generating_node;
}

cobalt::layout::Component::~Component(){
  printf("destroyed Component\n");
}

#if 0
bool cobalt::layout::Component::keyup(const scoped_refptr<dom::Event> &event,
