...
size_t Identity::Hash(size_t seed) const {
  ...
  seed = mojo::internal::WTFHash(seed, this->instance_group);
  seed = mojo::internal::WTFHash(seed, this->instance_id);
  seed = mojo::internal::WTFHash(seed, this->globally_unique_id);
  return seed;
}
...
ServiceInfo::ServiceInfo(
    const WTF::String& sandbox_type_in)
...
void ConnectorProxy::BindInterface(
    ::service_manager::mojom::blink::ServiceFilterPtr in_filter, const WTF::String& in_interface_name, mojo::ScopedMessagePipeHandle in_interface_pipe, BindInterfacePriority in_priority, BindInterfaceCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void ConnectorProxy::QueryService(
    const WTF::String& in_service_name, QueryServiceCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
