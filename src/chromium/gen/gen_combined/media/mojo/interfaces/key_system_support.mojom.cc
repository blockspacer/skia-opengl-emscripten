...
#include "media/mojo/interfaces/key_system_support.mojom-import-headers.h"
...
KeySystemCapability::KeySystemCapability(
    const std::vector<media::VideoCodec>& video_codecs_in,
    bool supports_vp9_profile2_in,
    const std::vector<media::EncryptionMode>& encryption_schemes_in,
    const std::vector<media::VideoCodec>& hw_secure_video_codecs_in,
    const std::vector<media::EncryptionMode>& hw_secure_encryption_schemes_in,
    const std::vector<media::CdmSessionType>& session_types_in)
...
bool KeySystemSupport::IsKeySystemSupported(const std::string& key_system, bool* out_is_supported, KeySystemCapabilityPtr* out_key_system_capability) {
  NOTREACHED();
  return false;
}
...
bool KeySystemSupportProxy::IsKeySystemSupported(
    const std::string& param_key_system, bool* out_param_is_supported, KeySystemCapabilityPtr* out_param_key_system_capability) {
