...
DevToolsSessionState::DevToolsSessionState(
    const base::flat_map<std::string, base::Optional<std::vector<uint8_t>>>& entries_in)
...
void DevToolsAgentHostProxy::ChildWorkerCreated(
    DevToolsAgentPtr in_worker_devtools_agent, DevToolsAgentHostRequest in_worker_devtools_agent_host, const GURL& in_url, const std::string& in_name, const base::UnguessableToken& in_devtools_worker_token, bool in_waiting_for_debugger) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void DevToolsSessionProxy::DispatchProtocolCommand(
    int32_t in_call_id, const std::string& in_method, DevToolsMessagePtr in_message) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
