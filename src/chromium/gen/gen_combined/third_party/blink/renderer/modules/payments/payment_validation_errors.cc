// Copyright 2014 The Chromium Authors. All rights reserved.
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

// This file has been auto-generated from the Jinja2 template
// third_party/blink/renderer/bindings/templates/dictionary_impl.cc.tmpl
// by the script code_generator_v8.py.
// DO NOT MODIFY!

// clang-format off
#include "third_party/blink/renderer/modules/payments/payment_validation_errors.h"

namespace blink {

PaymentValidationErrors::PaymentValidationErrors() {
}

PaymentValidationErrors::~PaymentValidationErrors() = default;

void PaymentValidationErrors::setPayer(PayerErrors* value) {
  payer_ = value;
}

void PaymentValidationErrors::setShippingAddress(AddressErrors* value) {
  shipping_address_ = value;
}

void PaymentValidationErrors::Trace(blink::Visitor* visitor) {
  visitor->Trace(payer_);
  visitor->Trace(shipping_address_);
  IDLDictionaryBase::Trace(visitor);
}

}  // namespace blink
