...
void V8TransitionEventInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, TransitionEventInit* impl, ExceptionState& exception_state) {
  ...
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&elapsed_time_value)) {
  ...
  v8::Local<v8::Value> property_name_value;
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&property_name_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (property_name_value.IsEmpty() || property_name_value->IsUndefined()) {
    // Do nothing.
  } else {
    V8StringResource<> property_name_cpp_value = property_name_value;
    if (!property_name_cpp_value.Prepare(exception_state))
      return;
    impl->setPropertyName(property_name_cpp_value);
  }
...
bool toV8TransitionEventInit(const TransitionEventInit* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (elapsed_time_has_value_or_default &&
      !create_property(0, elapsed_time_value)) {
    return false;
  }

  v8::Local<v8::Value> property_name_value;
  bool property_name_has_value_or_default = false;
  if (impl->hasPropertyName()) {
    property_name_value = V8String(isolate, impl->propertyName());
    property_name_has_value_or_default = true;
  } else {
    property_name_value = V8String(isolate, WTF::g_empty_string);
    property_name_has_value_or_default = true;
  }
  if (property_name_has_value_or_default &&
      !create_property(1, property_name_value)) {
    return false;
  }
...
TransitionEventInit* NativeValueTraits<TransitionEventInit>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  TransitionEventInit* impl = TransitionEventInit::Create();
  V8TransitionEventInit::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
