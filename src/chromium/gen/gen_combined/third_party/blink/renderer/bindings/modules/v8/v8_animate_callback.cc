...
v8::Maybe<void> V8AnimateCallback::Invoke(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, double currentTime, EffectProxyOrWorkletGroupEffectProxy effect) {
  ...
  v8::Local<v8::Value> v8_currentTime = v8::Number::New(GetIsolate(), currentTime);
  v8::Local<v8::Value> v8_effect = ToV8(effect, argument_creation_context, GetIsolate());
  constexpr int argc = 2;
  v8::Local<v8::Value> argv[] = { v8_currentTime, v8_effect };
...
void V8AnimateCallback::InvokeAndReportException(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, double currentTime, EffectProxyOrWorkletGroupEffectProxy effect) {
  v8::TryCatch try_catch(GetIsolate());
  try_catch.SetVerbose(true);

  v8::Maybe<void> maybe_result =
      Invoke(callback_this_value, currentTime, effect);
  // An exception if any is killed with the v8::TryCatch above.
  ALLOW_UNUSED_LOCAL(maybe_result);
}

v8::Maybe<void> V8PersistentCallbackFunction<V8AnimateCallback>::Invoke(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, double currentTime, EffectProxyOrWorkletGroupEffectProxy effect) {
  return Proxy()->Invoke(
      callback_this_value, currentTime, effect);
}

void V8PersistentCallbackFunction<V8AnimateCallback>::InvokeAndReportException(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, double currentTime, EffectProxyOrWorkletGroupEffectProxy effect) {
