...
void V8PublicKeyCredentialParameters::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, PublicKeyCredentialParameters* impl, ExceptionState& exception_state) {
  if (IsUndefinedOrNull(v8_value)) {
    exception_state.ThrowTypeError("Missing required member(s): alg, type.");
    return;
  }
  ...
  if (alg_value.IsEmpty() || alg_value->IsUndefined()) {
    exception_state.ThrowTypeError("required member alg is undefined.");
    return;
  } else {
    int32_t alg_cpp_value = NativeValueTraits<IDLLong>::NativeValue(isolate, alg_value, exception_state);
...
bool toV8PublicKeyCredentialParameters(const PublicKeyCredentialParameters* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasAlg()) {
    alg_value = v8::Integer::New(isolate, impl->alg());
...
PublicKeyCredentialParameters* NativeValueTraits<PublicKeyCredentialParameters>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  PublicKeyCredentialParameters* impl = PublicKeyCredentialParameters::Create();
  V8PublicKeyCredentialParameters::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
