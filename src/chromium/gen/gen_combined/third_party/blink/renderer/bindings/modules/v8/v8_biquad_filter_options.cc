...
void V8BiquadFilterOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, BiquadFilterOptions* impl, ExceptionState& exception_state) {
  ...
  if (!v8Object->Get(context, keys[0].Get(isolate)).ToLocal(&q_value)) {
  ...
  if (q_value.IsEmpty() || q_value->IsUndefined()) {
    ...
    float q_cpp_value = NativeValueTraits<IDLFloat>::NativeValue(isolate, q_value, exception_state);
    if (exception_state.HadException())
      return;
  ...
  v8::Local<v8::Value> gain_value;
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&gain_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (gain_value.IsEmpty() || gain_value->IsUndefined()) {
    // Do nothing.
  } else {
    float gain_cpp_value = NativeValueTraits<IDLFloat>::NativeValue(isolate, gain_value, exception_state);
    if (exception_state.HadException())
      return;
    impl->setGain(gain_cpp_value);
  }
  ...
  if (type_value.IsEmpty() || type_value->IsUndefined()) {
    ...
    if (!IsValidEnum(type_cpp_value, kValidValues, base::size(kValidValues), "BiquadFilterType", exception_state))
      return;
...
bool toV8BiquadFilterOptions(const BiquadFilterOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasQ()) {
    q_value = v8::Number::New(isolate, impl->Q());
    q_has_value_or_default = true;
  } else {
    q_value = v8::Number::New(isolate, 1);
    q_has_value_or_default = true;
  }
  if (q_has_value_or_default &&
      !create_property(0, q_value)) {
    return false;
  }
  ...
  if (impl->hasFrequency()) {
    ...
    frequency_value = v8::Number::New(isolate, 350);
    frequency_has_value_or_default = true;
  }
  ...
  v8::Local<v8::Value> gain_value;
  bool gain_has_value_or_default = false;
  if (impl->hasGain()) {
    gain_value = v8::Number::New(isolate, impl->gain());
    gain_has_value_or_default = true;
  } else {
    gain_value = v8::Number::New(isolate, 0);
    gain_has_value_or_default = true;
  }
  if (gain_has_value_or_default &&
      !create_property(3, gain_value)) {
    return false;
  }
  ...
  if (impl->hasType()) {
    ...
    type_value = V8String(isolate, "lowpass");
    type_has_value_or_default = true;
  }
...
BiquadFilterOptions* NativeValueTraits<BiquadFilterOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  BiquadFilterOptions* impl = BiquadFilterOptions::Create();
  V8BiquadFilterOptions::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
