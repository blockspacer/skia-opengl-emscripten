...
MediaMetadata::MediaMetadata(
    ...
    const base::string16& source_title_in)
...
size_t MediaSessionInfo::Hash(size_t seed) const {
  ...
  seed = mojo::internal::Hash(seed, this->force_duck);
  seed = mojo::internal::Hash(seed, this->playback_state);
  seed = mojo::internal::Hash(seed, this->is_controllable);
  seed = mojo::internal::Hash(seed, this->prefer_stop_for_gain_focus_loss);
  return seed;
}
...
void MediaSessionObserverProxy::MediaSessionMetadataChanged(
    const base::Optional<media_session::MediaMetadata>& in_metadata) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
std::move(p_info));
    ...
    case internal::kMediaSessionObserver_MediaSessionMetadataChanged_Name: {
      ...
      serialization_context.TakeHandlesFromMessage(message);
      bool success = true;
      base::Optional<media_session::MediaMetadata> p_metadata{};
