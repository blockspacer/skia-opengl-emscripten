...
void WebSocketProxy::AddChannelRequest(
    const GURL& in_url, const std::vector<std::string>& in_requested_protocols, const GURL& in_first_party_for_cookies, std::vector<HttpHeaderPtr> in_additional_headers, WebSocketClientPtr in_client) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
bool WebSocketStubDispatch::Accept(
  ...
  switch (message->header()->name) {
    case internal::kWebSocket_AddChannelRequest_Name: {
      ...
      std::vector<HttpHeaderPtr> p_additional_headers{};
