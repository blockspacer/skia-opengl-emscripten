...
bool FileSystemManager::CreateSnapshotFile(const ::blink::KURL& file_path, base::File::Info* out_file_info, base::FilePath* out_platform_path, ::base::File::Error* out_error_code, ReceivedSnapshotListenerPtr* out_snapshot_listener) {
  NOTREACHED();
  return false;
}
...
void FileSystemManagerProxy::ReadDirectory(
    const ::blink::KURL& in_path, FileSystemOperationListenerPtr in_listener) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void FileSystemManagerProxy::Write(
    const ::blink::KURL& in_file_path, const WTF::String& in_blob_uuid, int64_t in_position, FileSystemCancellableOperationRequest in_op_request, FileSystemOperationListenerPtr in_listener) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void FileSystemManagerProxy::Truncate(
    const ::blink::KURL& in_file_path, int64_t in_length, FileSystemCancellableOperationRequest in_op_request, TruncateCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void FileSystemManagerProxy::TouchFile(
    const ::blink::KURL& in_path, base::Time in_last_access_time, base::Time in_last_modified_time, TouchFileCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
bool FileSystemManagerProxy::CreateSnapshotFile(
    const ::blink::KURL& param_file_path, base::File::Info* out_param_file_info, base::FilePath* out_param_platform_path, ::base::File::Error* out_param_error_code, ReceivedSnapshotListenerPtr* out_param_snapshot_listener) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
