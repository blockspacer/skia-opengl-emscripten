...
void BadgeServiceInterceptorForTesting::SetInteger(uint64_t content) {
  GetForwardingInterface()->SetInteger(std::move(content));
}
void BadgeServiceInterceptorForTesting::SetFlag() {
  GetForwardingInterface()->SetFlag();
}
