...
class BLINK_COMMON_EXPORT CacheStorageCacheInterceptorForTesting : public CacheStorageCache {
  ...
  void Match(::blink::mojom::FetchAPIRequestPtr request, CacheQueryOptionsPtr query_options, int64_t trace_id, MatchCallback callback) override;
  ...
  void SetSideData(const GURL& url, base::Time response_time, const std::vector<uint8_t>& side_data, int64_t trace_id, SetSideDataCallback callback) override;
};
...
class BLINK_COMMON_EXPORT CacheStorageInterceptorForTesting : public CacheStorage {
  ...
  void Has(const base::string16& cache_name, int64_t trace_id, HasCallback callback) override;
  void Delete(const base::string16& cache_name, int64_t trace_id, DeleteCallback callback) override;
  void Keys(int64_t trace_id, KeysCallback callback) override;
  void Match(::blink::mojom::FetchAPIRequestPtr request, MultiCacheQueryOptionsPtr match_options, int64_t trace_id, MatchCallback callback) override;
  void Open(const base::string16& cache_name, int64_t trace_id, OpenCallback callback) override;
};
