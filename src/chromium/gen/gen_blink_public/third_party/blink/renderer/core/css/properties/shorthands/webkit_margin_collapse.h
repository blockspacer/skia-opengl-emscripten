...
class WebkitMarginCollapse final : public Shorthand {
 public:
  constexpr WebkitMarginCollapse() : Shorthand(CSSPropertyID::kWebkitMarginCollapse,
    0
    | kProperty
    ,
    '\0'
  ...
  const char* GetPropertyName() const override { return "-webkit-margin-collapse"; }
  const WTF::AtomicString& GetPropertyNameAtomicString() const override {
    DEFINE_STATIC_LOCAL(const AtomicString, name, ("-webkit-margin-collapse"));
    return name;
  }
  const char* GetJSPropertyName() const override {
    return "webkitMarginCollapse";
  }
