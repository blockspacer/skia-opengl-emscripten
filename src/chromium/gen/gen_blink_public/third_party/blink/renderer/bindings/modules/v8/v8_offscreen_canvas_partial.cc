...
namespace offscreen_canvas_partial_v8_internal {

static void GetContextMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exception_state(info.GetIsolate(), ExceptionState::kExecutionContext, "OffscreenCanvas", "getContext");
  ...
  V8StringResource<> context_type;
  CanvasContextCreationAttributesModule* attributes;
  context_type = info[0];
  if (!context_type.Prepare())
    return;
  const char* kValidContextTypeValues[] = {
      "2d",
      "webgl",
      "webgl2",
      "webgl2-compute",
  };
  if (!IsValidEnum(context_type, kValidContextTypeValues, base::size(kValidContextTypeValues), "OffscreenRenderingContextType", exception_state)) {
    return;
  }

  if (!info[1]->IsNullOrUndefined() && !info[1]->IsObject()) {
    exception_state.ThrowTypeError("parameter 2 ('attributes') is not an object.");
    return;
  }
  ...
  OffscreenCanvasRenderingContext2DOrWebGLRenderingContextOrWebGL2RenderingContextOrWebGL2ComputeRenderingContext result;
  OffscreenCanvasModule::getContext(execution_context, *impl, context_type, attributes, exception_state, result);
  if (exception_state.HadException()) {
    return;
  }
...
void V8OffscreenCanvasPartial::GetContextMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  offscreen_canvas_partial_v8_internal::GetContextMethod(info);
}
...
void V8OffscreenCanvasPartial::InstallV8OffscreenCanvasTemplate(
  ...
  V8OffscreenCanvas::InstallV8OffscreenCanvasTemplate(isolate, world, interface_template);
...
void V8OffscreenCanvasPartial::InstallRuntimeEnabledFeaturesOnTemplate(
  ...
  if (RuntimeEnabledFeatures::OffscreenCanvasEnabled()) {
    {
      // Install getContext configuration
