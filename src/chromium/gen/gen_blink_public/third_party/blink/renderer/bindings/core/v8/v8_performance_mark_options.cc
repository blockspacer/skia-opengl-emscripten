...
void V8PerformanceMarkOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, PerformanceMarkOptions* impl, ExceptionState& exception_state) {
  ...
  if (!v8Object->Get(context, keys[1].Get(isolate)).ToLocal(&start_time_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  if (start_time_value.IsEmpty() || start_time_value->IsUndefined()) {
    ...
    double start_time_cpp_value = NativeValueTraits<IDLDouble>::NativeValue(isolate, start_time_value, exception_state);
...
bool toV8PerformanceMarkOptions(const PerformanceMarkOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasStartTime()) {
    start_time_value = v8::Number::New(isolate, impl->startTime());
    start_time_has_value_or_default = true;
  }
  if (start_time_has_value_or_default &&
      !create_property(1, start_time_value)) {
    return false;
  }
...
PerformanceMarkOptions* NativeValueTraits<PerformanceMarkOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  PerformanceMarkOptions* impl = PerformanceMarkOptions::Create();
  V8PerformanceMarkOptions::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
