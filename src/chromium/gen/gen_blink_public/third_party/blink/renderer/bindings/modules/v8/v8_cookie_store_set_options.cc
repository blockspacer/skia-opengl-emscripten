...
void V8CookieStoreSetOptions::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, CookieStoreSetOptions* impl, ExceptionState& exception_state) {
  ...
  if (!v8Object->Get(context, keys[3].Get(isolate)).ToLocal(&same_site_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
  ...
  if (!v8Object->Get(context, keys[4].Get(isolate)).ToLocal(&secure_value)) {
    exception_state.RethrowV8Exception(block.Exception());
    return;
  }
...
bool toV8CookieStoreSetOptions(const CookieStoreSetOptions* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (same_site_has_value_or_default &&
      !create_property(3, same_site_value)) {
    return false;
  }
  ...
  if (secure_has_value_or_default &&
      !create_property(4, secure_value)) {
    return false;
  }
...
CookieStoreSetOptions* NativeValueTraits<CookieStoreSetOptions>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  CookieStoreSetOptions* impl = CookieStoreSetOptions::Create();
  V8CookieStoreSetOptions::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
