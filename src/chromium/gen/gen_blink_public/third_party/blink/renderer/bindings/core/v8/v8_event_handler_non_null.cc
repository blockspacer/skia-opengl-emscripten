...
v8::Maybe<ScriptValue> V8EventHandlerNonNull::Invoke(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, const Vector<ScriptValue>& args) {
  ...
  // zero.
  const int argc = 0 + args.size();
  v8::Local<v8::Value> argv[std::max(1, argc)];
  for (wtf_size_t i = 0; i < args.size(); ++i) {
    argv[0 + i] = ToV8(args[i], argument_creation_context, GetIsolate());
  }
...
v8::Maybe<ScriptValue> V8EventHandlerNonNull::Construct(const Vector<ScriptValue>& args) {
  ScriptState* callback_relevant_script_state =
      CallbackRelevantScriptStateOrThrowException(
          "EventHandlerNonNull",
          "construct");
...
v8::Maybe<ScriptValue> V8EventHandlerNonNull::InvokeWithoutRunnabilityCheck(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, const Vector<ScriptValue>& args) {
  ScriptState* callback_relevant_script_state =
      CallbackRelevantScriptStateOrThrowException(
          "EventHandlerNonNull",
          "invoke");
...
v8::Maybe<ScriptValue> V8PersistentCallbackFunction<V8EventHandlerNonNull>::Invoke(bindings::V8ValueOrScriptWrappableAdapter callback_this_value, const Vector<ScriptValue>& args) {
  return Proxy()->Invoke(
      callback_this_value, args);
}
