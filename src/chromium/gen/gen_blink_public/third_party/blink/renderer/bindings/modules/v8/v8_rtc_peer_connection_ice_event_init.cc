...
void V8RTCPeerConnectionIceEventInit::ToImpl(v8::Isolate* isolate, v8::Local<v8::Value> v8_value, RTCPeerConnectionIceEventInit* impl, ExceptionState& exception_state) {
  ...
  if (candidate_value.IsEmpty() || candidate_value->IsUndefined()) {
    // Do nothing.
  } else if (candidate_value->IsNull()) {
    impl->setCandidateToNull();
  } else {
    RTCIceCandidate* candidate_cpp_value = V8RTCIceCandidate::ToImplWithTypeCheck(isolate, candidate_value);
    if (!candidate_cpp_value) {
      exception_state.ThrowTypeError("member candidate is not of type RTCIceCandidate.");
      return;
    }
...
bool toV8RTCPeerConnectionIceEventInit(const RTCPeerConnectionIceEventInit* impl, v8::Local<v8::Object> dictionary, v8::Local<v8::Object> creationContext, v8::Isolate* isolate) {
  ...
  if (impl->hasCandidate()) {
    candidate_value = ToV8(impl->candidate(), creationContext, isolate);
...
RTCPeerConnectionIceEventInit* NativeValueTraits<RTCPeerConnectionIceEventInit>::NativeValue(v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  RTCPeerConnectionIceEventInit* impl = RTCPeerConnectionIceEventInit::Create();
  V8RTCPeerConnectionIceEventInit::ToImpl(isolate, value, impl, exception_state);
  return impl;
}
