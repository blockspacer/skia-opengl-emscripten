...
static void GetPoseMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (UNLIKELY(info.Length() < 1)) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("getPose", "XRPlane", ExceptionMessages::NotEnoughArguments(1, info.Length())));
    return;
  }
  ...
  if (!reference_space) {
    V8ThrowException::ThrowTypeError(info.GetIsolate(), ExceptionMessages::FailedToExecute("getPose", "XRPlane", "parameter 1 is not of type 'XRReferenceSpace'."));
    return;
  }

  V8SetReturnValue(info, impl->getPose(reference_space));
}
...
void V8XRPlane::GetPoseMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  RUNTIME_CALL_TIMER_SCOPE_DISABLED_BY_DEFAULT(info.GetIsolate(), "Blink_XRPlane_getPose");

  xr_plane_v8_internal::GetPoseMethod(info);
}
...
XRPlane* NativeValueTraits<XRPlane>::NativeValue(
    v8::Isolate* isolate, v8::Local<v8::Value> value, ExceptionState& exception_state) {
  XRPlane* native_value = V8XRPlane::ToImplWithTypeCheck(isolate, value);
  if (!native_value) {
    ...
    if (is_secure_context) {
      {
        ...
        const V8DOMConfiguration::MethodConfiguration kConfigurations[] = {
            {"getPose", V8XRPlane::GetPoseMethodCallback, 1, v8::None, V8DOMConfiguration::kOnPrototype, V8DOMConfiguration::kCheckHolder, V8DOMConfiguration::kDoNotCheckAccess, V8DOMConfiguration::kHasSideEffect, V8DOMConfiguration::kAllWorlds}
        };
