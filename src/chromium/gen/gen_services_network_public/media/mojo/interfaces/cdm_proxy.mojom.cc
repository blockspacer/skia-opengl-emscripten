...
#include "media/mojo/interfaces/cdm_proxy.mojom-import-headers.h"
...
void CdmProxyProxy::SetKey(
    uint32_t in_crypto_session_id, const std::vector<uint8_t>& in_key_id, media::CdmProxy::KeyType in_key_type, const std::vector<uint8_t>& in_key_blob, SetKeyCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
void CdmProxyProxy::RemoveKey(
    uint32_t in_crypto_session_id, const std::vector<uint8_t>& in_key_id, RemoveKeyCallback callback) {
#if BUILDFLAG(MOJO_TRACE_ENABLED)
...
class CdmProxy_SetKey_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      media::CdmProxy::Status in_status);
...
std::move(p_input_data), std::move(callback));
    ...
    case internal::kCdmProxy_SetKey_Name: {
      ...
      media::CdmProxy::KeyType p_key_type{};
