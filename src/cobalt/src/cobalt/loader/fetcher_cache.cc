...
class CachedFetcherHandler : public Fetcher::Handler {
 ...
 private:
  ...
  LoadResponseType OnResponseStarted(
    ...
    DCHECK_CALLED_ON_VALID_THREAD(thread_checker_);
    DCHECK(wrapping_fetcher_);
    auto response = handler_->OnResponseStarted(wrapping_fetcher_
#if defined(ENABLE_GNET)
    , headers
#endif
    );
  ...
  void OnDone(Fetcher*) override {
    ...
    on_success_callback_.Run(
        ...
        , headers_
#endif
        , wrapping_fetcher_->last_url_origin(),
...
class CachedFetcher : public Fetcher {
 public:
  CachedFetcher(const GURL& url,
    ...
    auto response_type = handler->OnResponseStarted(this
#if defined(ENABLE_GNET)
    , headers
#endif
    );
