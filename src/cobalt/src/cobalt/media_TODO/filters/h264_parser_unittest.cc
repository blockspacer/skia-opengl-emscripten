...
TEST_F(H264SPSTest, GetCodedSize) {
  ...
  EXPECT_EQ(cobalt::math::Size(848, 480), sps->GetCodedSize());
...
TEST_F(H264SPSTest, GetVisibleRect) {
  std::unique_ptr<H264SPS> sps = MakeSPS_BBB480p();
  EXPECT_EQ(cobalt::math::Rect(0, 0, 848, 480), sps->GetVisibleRect());
  ...
  EXPECT_EQ(cobalt::math::Rect(2, 6, 848 - 6, 480 - 14), sps->GetVisibleRect());
  ...
  EXPECT_EQ(cobalt::math::Rect(844, 0, 2, 480), sps->GetVisibleRect());
