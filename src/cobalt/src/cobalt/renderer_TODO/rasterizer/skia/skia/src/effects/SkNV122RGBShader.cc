...
void SkNV122RGBShader::InitializeShaders() {
  ...
  uv_shader_ = (SkImageShader::Make(uv_image_, SkTileMode::kClamp,
                                     SkTileMode::kClamp, &uv_matrix_));
  DCHECK(uv_shader_);
}
...
/*sk_sp<SkFlattenable> SkNV122RGBShader::CreateProc(SkReadBuffer& buffer) {
  ...
  sk_sp<SkImage> y_image = buffer.readImage();//BitmapAsImage();
  if (!y_image) {
    return nullptr;
  }
  SkMatrix y_matrix;
  ...
  sk_sp<SkImage> uv_image = buffer.readImage();//buffer.readBitmapAsImage();
  if (!uv_image) {
    return nullptr;
  }
  SkMatrix uv_matrix;
