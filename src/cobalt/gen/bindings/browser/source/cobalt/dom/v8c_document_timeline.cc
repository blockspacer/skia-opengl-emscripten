...
void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  scoped_refptr<DocumentTimeline> new_object =
      new DocumentTimeline(callwith_global_environment->GetEnvironmentSettings(), originTime);
...
void InitializeTemplate(v8::Isolate* isolate) {
  ...
  function_template->SetClassName(NewInternalString(isolate, "DocumentTimeline"));
...

  {
    ...
    // inherited interface.
    v8::Local<v8::FunctionTemplate> parent_template = V8cAnimationTimeline::GetTemplate(isolate);
    function_template->Inherit(parent_template);
    static_assert(
        std::is_base_of<AnimationTimeline, DocumentTimeline>::value,
        "Expected DocumentTimeline to have C++ parent class "
        "AnimationTimeline, because that is its WebIDL parent.");
  }
  ...
  prototype_template->Set(
      ...
      NewInternalString(isolate, "DocumentTimeline"),
...
v8::Local<v8::Object> V8cDocumentTimeline::CreateWrapper(
...
v8::Local<v8::FunctionTemplate> V8cDocumentTimeline::GetTemplate(v8::Isolate* isolate) {
  V8cGlobalEnvironment* global_environment = V8cGlobalEnvironment::GetFromIsolate(isolate);
  if (!global_environment->HasInterfaceData(kInterfaceUniqueId)) {
    InitializeTemplate(isolate);
  }
