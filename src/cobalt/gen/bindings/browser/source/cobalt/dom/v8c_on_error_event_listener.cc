...
base::Optional<bool > V8cOnErrorEventListener::HandleEvent(
  ...
  bool success = false;
  base::Optional<bool > cobalt_return_value;
  ...
  DCHECK(!this->IsEmpty());
  {
    ...
    if (!GetCallableForCallbackInterface(
             ...
             NewInternalString(isolate_, "handleEvent"))
             .ToLocal(&callable)) {
      goto done;
    }
    ...
    ToJSValue(isolate_, event, &argv[0]);
    ToJSValue(isolate_, filename, &argv[1]);
    ToJSValue(isolate_, lineno, &argv[2]);
    ToJSValue(isolate_, colno, &argv[3]);
    ToJSValue(isolate_, error, &argv[4]);
