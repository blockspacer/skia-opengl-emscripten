...
const int kInterfaceUniqueId = 205;
...
void Constructor(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  scoped_refptr<Text> new_object =
      new Text(callwith_global_environment->GetEnvironmentSettings(), data);
...
void InitializeTemplate(v8::Isolate* isolate) {
  ...
  function_template->SetClassName(NewInternalString(isolate, "Text"));
...

  {
    ...
    // inherited interface.
    v8::Local<v8::FunctionTemplate> parent_template = V8cCharacterData::GetTemplate(isolate);
    function_template->Inherit(parent_template);
    static_assert(
        std::is_base_of<CharacterData, Text>::value,
        "Expected Text to have C++ parent class "
        "CharacterData, because that is its WebIDL parent.");
  }
  ...
  prototype_template->Set(
      ...
      NewInternalString(isolate, "Text"),
...
v8::Local<v8::Object> V8cText::CreateWrapper(
...
v8::Local<v8::FunctionTemplate> V8cText::GetTemplate(v8::Isolate* isolate) {
  V8cGlobalEnvironment* global_environment = V8cGlobalEnvironment::GetFromIsolate(isolate);
  if (!global_environment->HasInterfaceData(kInterfaceUniqueId)) {
    InitializeTemplate(isolate);
  }
