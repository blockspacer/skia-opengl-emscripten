...
void ToJSValue(
    v8::Isolate* isolate,
    const BlobEventInit& in_dictionary,
  ...
  if (in_dictionary.has_timecode()) {
    v8::Local<v8::Value> member_value;
    ToJSValue(isolate, in_dictionary.timecode(), &member_value);
    v8::Maybe<bool> maybe_result = dictionary_object->DefineOwnProperty(
      context,
      v8::String::NewFromUtf8(
        isolate,
...
void FromJSValue(v8::Isolate* isolate, v8::Local<v8::Value> value,
...

  {
    ...
    if (!member_value->IsUndefined()) {
      scoped_refptr<Blob> converted_value;
      FromJSValue(isolate,
                  member_value,
                  kNoConversionFlags,
                  exception_state,
  ...
  }
  {
    v8::MaybeLocal<v8::Value> maybe_member_value = dictionary_object->Get(
        context,
        v8::String::NewFromUtf8(isolate, "timecode", v8::NewStringType::kInternalized).ToLocalChecked());
    v8::Local<v8::Value> member_value;
    if (!maybe_member_value.ToLocal(&member_value)) {
      exception_state->SetSimpleException(kSimpleError);
      return;
    ...
    if (!member_value->IsUndefined()) {
      ...
      FromJSValue(isolate,
                  ...
                  exception_state,
                  &converted_value);
      out_dictionary->set_timecode(converted_value);
    }
  }
}
