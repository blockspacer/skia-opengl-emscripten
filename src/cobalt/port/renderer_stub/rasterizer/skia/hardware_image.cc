...
#include "renderer_stub/backend/egl/framebuffer_render_target.h"
...
#include "renderer_stub/rasterizer/skia/gl_format_conversions.h"
...
HardwareFrontendImage::~HardwareFrontendImage() {
  ...
  if (rasterizer_message_loop_) {
    if (rasterizer_message_loop_->task_runner() != base::MessageLoopCurrent::Get().task_runner() ||
...
const sk_sp<SkImage>& HardwareFrontendImage::GetImage() const {
  DCHECK_EQ(rasterizer_message_loop_->task_runner(), base::MessageLoopCurrent::Get().task_runner());
