...
/*void DestructSubmissionOnMessageLoop(base::MessageLoop* message_loop,
  ...
  //if (base::MessageLoopCurrent::Get() != message_loop) {
  if (base::MessageLoopCurrent::Get().task_runner() != message_loop->task_runner()) {
    message_loop->task_runner()->DeleteSoon(FROM_HERE, submission.release());
  }
}*/

void DestructSubmissionOnMessageLoop(scoped_refptr<base::SingleThreadTaskRunner> message_runner,
                                     std::unique_ptr<Submission> submission) {
    TRACE_EVENT0("cobalt::renderer", "DestructSubmissionOnMessageLoop()");
    //if (base::MessageLoop::current() != message_loop) {
    //if (base::MessageLoopCurrent::Get() != message_loop) {
    if (base::MessageLoopCurrent::Get().task_runner() != message_runner) {
        message_runner->DeleteSoon(FROM_HERE, submission.release());
    }
}
}  // namespace
...
Pipeline::Pipeline(const CreateRasterizerFunction& create_rasterizer_function,
  ...
  //rasterizer_thread_.message_loop()->task_runner()->PostTask(
    ...
    //  base::Bind(&Pipeline::InitializeRasterizerThread, base::Unretained(this),
    //             create_rasterizer_function));
    std::move(base::Bind(&Pipeline::InitializeRasterizerThread, base::Unretained(this),
                 create_rasterizer_function)).Run();
#endif
...
Pipeline::~Pipeline() {
  ...
  //    FROM_HERE,
  //    base::Bind(&Pipeline::ShutdownSubmissionQueue, base::Unretained(this)));
  std::move(base::Bind(&Pipeline::ShutdownSubmissionQueue, base::Unretained(this))).Run();
#endif
...
void Pipeline::Submit(const Submission& render_tree_submission) {
  ...
  //rasterizer_thread_.message_loop()->task_runner()->PostTask(
  DCHECK(rasterizer_thread_.task_runner());
  rasterizer_thread_.task_runner()->PostTask(
...
void Pipeline::Clear() {
  ...
  //    FROM_HERE,
  //    base::Bind(&Pipeline::ClearCurrentRenderTree, base::Unretained(this)));
  std::move(base::Bind(&Pipeline::ClearCurrentRenderTree, base::Unretained(this))).Run();
#endif
}
...
void Pipeline::RasterizeCurrentTree() {
  ...
  Submission submission =
    ...
    //printf("Pipeline::RasterizeCurrentTree 3\n");
  ...
  if (minimum_fps > 0) {
    ...
    //printf("Pipeline::RasterizeCurrentTree 6\n");
  ...
  if (force_rasterize || has_render_tree_changed) {
    //printf("Pipeline::RasterizeCurrentTree 7\n");
    ...
    //printf("Pipeline::RasterizeCurrentTree 8\n");
  }
