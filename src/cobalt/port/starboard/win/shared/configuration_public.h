...
// The Starboard configuration for Desktop win. Other devices will have
// specific Starboard implementations, even if they ultimately are running some
// version of win -- but they may base their configuration on the Desktop
// win configuration headers.
...
// Desktop win needs a using statement for the hash functions.
...
// On default win desktop, you must be a superuser in order to set real time
...
// not be called. Use this for OSs (such as win) where receive buffer
...
/*#if !defined(__GNUC__)
#error "win builds need a GCC-like compiler (for the moment)."
#endif*/